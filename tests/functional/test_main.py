...
def test_register_and_login(test_client, celery_mock):
    ...
    # 登入
    response = test_client.post(
        "/zh/login", data={"account": "jason@example.com", "password": "cat", "submit": "Login"}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_change_password(test_client):
    ...
    response = test_client.post(
        "/zh/change_password",
        data={"old_password": "test1", "password": "test1", "password2": "test1", "submit": "Update"},
        follow_redirects=True,
    )
    assert response.status_code == 200
...
def test_user_role(test_client):
    ...
    # 更新使用者權限
    response = test_client.post(
        "/zh/UserRole", data={"submit": "Submit", "uid": 1, "username": "test1", "userrole": 3}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_user_upload_and_sort(test_client):
    ...
    # 刪除自己上傳的資料
    response = test_client.post(
        f"/zh/user/upload/test1/1", data={"delete": "Delete", "delete_mid": 1}, follow_redirects=True
    )
    assert response.status_code == 200
